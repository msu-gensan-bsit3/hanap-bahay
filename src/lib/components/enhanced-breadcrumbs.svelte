<script lang="ts">
	...
	let {
		...
		additionalContext?: { propertyName?: string };
	} = $props();
	...
	const breadcrumbItems = $derived(() => {
		...
		segments.forEach((segment, index) => {
			...
			if (segments[index - 1] === "listings" && /^\d+$/.test(segment)) {
				label = additionalContext?.propertyName || "Property Details";
			}
				...
				<Breadcrumb.List>
					{#each breadcrumbItems() as item, index}
						<Breadcrumb.Item>
							...
							{:else}
								<!-- Linkable breadcrumb -->
								<Breadcrumb.Link href={item.href} class="flex items-center gap-2">
									{#if item.isHome}
