...
<header class="sticky top-0 z-50 h-16 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60" bind:clientHeight={heights.header}>
	<div class="mx-auto flex h-full max-w-screen-xl items-center justify-between px-4 sm:px-6 lg:px-8">
		<!-- Logo -->
		...
		<nav class="hidden items-center gap-8 md:flex">
			...
			<div class="flex items-center gap-4">
					...
					<DropdownMenu.Root>
						<DropdownMenu.Trigger class="flex items-center gap-2 rounded-md p-2 hover:bg-accent">
							<Avatar.Root class="h-8 w-8">
								<Avatar.Image src={user.profilePicture || undefined} alt={`${user.firstName} ${user.lastName}`} />
								<Avatar.Fallback>
				...
				{:else}
					<!-- Not logged in -->
					<Button variant="ghost" href="/login" class="text-sm font-medium">
						Sign In
					</Button>
					<Button href="/register" class="text-sm font-medium">
						Sign Up
					</Button>
				{/if}
			</div>
		...
		<div class="flex items-center gap-4 md:hidden">
			...
			{:else}
				...
				<Button variant="ghost" href="/login" size="sm" class="text-sm">
					Sign In
				</Button>
			{/if}
			
			<DropdownMenu.Root bind:open={mobileMenuOpen}>
				<DropdownMenu.Trigger class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-input bg-background p-0 text-sm font-medium ring-offset-background transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50">
					{#if mobileMenuOpen}
						<X class="h-5 w-5" />
					{:else}
						<Menu class="h-5 w-5" />
				...
				<DropdownMenu.Content align="end" class="w-56">
					<DropdownMenu.Item onSelect={handleItemSelect("/")}>
						Buy
					</DropdownMenu.Item>
					<DropdownMenu.Item onSelect={handleItemSelect("/")}>
						Rent
					...
					{#if !user}
						<DropdownMenu.Separator />
						<DropdownMenu.Item onSelect={handleItemSelect("/register")}>
							Sign Up
						</DropdownMenu.Item>
					{/if}
				</DropdownMenu.Content>
