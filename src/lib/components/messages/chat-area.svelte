...
<div class="flex flex-1 flex-col">
	<Card class="flex h-full flex-col">
		...
		<CardHeader class="border-b pb-4">
			<div class="flex items-center gap-3">
				...
				<div class="flex-1 min-w-0">
					...
					<p class="text-sm text-blue-600 font-medium truncate">{selectedConversation.property}</p>
				...
				<div class="flex gap-2">
					...
					<Button variant="outline" size="sm" class="hidden sm:flex">
						...
						<span class="hidden lg:inline ml-1">Call</span>
					</Button>
					<Button variant="outline" size={isMobile ? "icon" : "sm"}>
						<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
						</svg>
						{#if !isMobile}<span class="hidden lg:inline ml-1">Info</span>{/if}
					</Button>
					{#if isMobile}
		...
		<!-- Messages Area -->
		<CardContent class="flex-1 overflow-y-auto p-3 lg:p-4 space-y-3 lg:space-y-4">
							...
							<div class="flex items-end gap-2 justify-end">
								<div class="min-w-0">
									<div class="rounded-2xl bg-blue-600 px-3 py-2 lg:px-4">
										<p class="text-sm text-white break-words">{message.content}</p>
									</div>
									<p class="mt-1 text-xs text-gray-500 text-right">{message.timestamp}</p>
								</div>
							</div>
						{/if}
					</div>
		...
		<div class="border-t p-3 lg:p-4">
			<div class="flex gap-2 lg:gap-3">
				<Button variant="outline" size="icon" class="shrink-0 hidden sm:flex">
					<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
