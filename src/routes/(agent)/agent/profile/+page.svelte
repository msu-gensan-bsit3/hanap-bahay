<script lang="ts">
	...
	let editForm = $state({
		...
		prcLicenceNumber: agent.prcLicenceNumber || "",
	});
	...
	async function handleSave() {
		...
		try {
			...
			console.log("Saving agent profile changes:", editForm);
	...
	function getVerificationStatus() {
		return agent.prcLicenceNumber ? "VERIFIED" : "PENDING";
	}

	function getExperienceYears() {
		const joinDate = new Date(user.dateCreated);
		const now = new Date();
		const years = now.getFullYear() - joinDate.getFullYear();
		return years > 0 ? years.toString() : "< 1";
	}
</script>

<svelte:head>
	<title>Agent Profile - JuanHomes</title>
</svelte:head>
...
<div class="container mx-auto max-w-6xl p-6">
	<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
		<div class="space-y-2">
			<h1 class="text-3xl font-bold tracking-tight">Agent Profile</h1>
			<p class="text-muted-foreground">Manage your professional information and credentials</p>
		</div>
			...
			<Card class="relative overflow-hidden py-0 transition-all duration-300 hover:shadow-lg">
				...
				<div class="relative -mt-12 flex flex-col items-center px-6 pb-6 text-center">
					...
					<div class="group relative">
						...
						<div
							class="absolute -right-1 -bottom-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-background {getVerificationStatus() ===
							'VERIFIED'
								? 'bg-green-500'
								: 'bg-yellow-500'}"
						>
							<Shield class="m-0.5 h-3 w-3 text-white" />
						</div>
					</div>
					...
					<div class="mt-4 w-full space-y-3">
						...
						<div class="flex flex-wrap justify-center gap-2">
							...
							<Badge
								variant={getVerificationStatus() === "VERIFIED" ? "default" : "secondary"}
								class="px-3 py-1 {getVerificationStatus() === 'VERIFIED'
									? 'bg-green-600 hover:bg-green-700'
									: 'bg-yellow-600 hover:bg-yellow-700'}"
							>
								{getVerificationStatus() === "VERIFIED" ? "Verified Agent" : "Pending Verification"}
							</Badge>
						</div>
					</div>
						...
						<div class="space-y-3 text-sm">
							...
							>
								...
								<span class="flex-1 text-left"
									>{getExperienceYears()} year{getExperienceYears() !== "1" &&
		...
		<div class="lg:col-span-2">
			<Card class="overflow-hidden py-0 transition-all duration-300 hover:shadow-lg">
				<div class="border-b bg-muted/30 px-6 py-4">
					<h3 class="text-lg font-semibold">Professional Profile</h3>
				...
				<div class="p-6">
					<div class="space-y-8 transition-all duration-300">
						...
						<div class="space-y-4">
							...
							<div class="space-y-6">
								<div class="space-y-2">
									<Label for="prcLicenceNumber" class="text-sm font-medium"
										>PRC License Number</Label
									>
									{#if isEditing}
										<Input
											id="prcLicenceNumber"
											bind:value={editForm.prcLicenceNumber}
									...
									{:else}
										<div
											class="rounded-lg border bg-muted/30 px-4 py-3 text-sm transition-all duration-200 hover:scale-[1.01] hover:bg-muted/50"
										>
											{agent.prcLicenceNumber || "Not provided"}
										</div>
									{/if}
									<p class="text-xs text-muted-foreground">
										Required for verification as a licensed real estate professional
									</p>
								</div>

								<div class="space-y-2">
									<Label for="about" class="text-sm font-medium">About / Bio</Label>
									{#if isEditing}
										<Textarea
											id="about"
											bind:value={editForm.about}
											placeholder="Tell potential clients about yourself, your experience, and specialization..."
											class="min-h-[100px] transition-all duration-200 focus:scale-[1.02] focus:ring-2 focus:ring-primary/20"
										/>
									{:else}
										<div
											class="min-h-[100px] rounded-lg border bg-muted/30 px-4 py-3 text-sm whitespace-pre-wrap transition-all duration-200 hover:scale-[1.01] hover:bg-muted/50"
										>
											{agent.about || "No bio provided"}
										</div>
									{/if}
								</div>
								...
								<div class="space-y-2">
									<Label for="credentials" class="text-sm font-medium"
										>Credentials & Certifications</Label
									>
									{#if isEditing}
										<Textarea
											id="credentials"
											bind:value={editForm.credentials}
											placeholder="List your professional credentials, certifications, awards, or achievements..."
											class="min-h-[80px] transition-all duration-200 focus:scale-[1.02] focus:ring-2 focus:ring-primary/20"
										/>
									{:else}
										<div
											class="min-h-[80px] rounded-lg border bg-muted/30 px-4 py-3 text-sm whitespace-pre-wrap transition-all duration-200 hover:scale-[1.01] hover:bg-muted/50"
										>
											{agent.credentials || "No credentials listed"}
										</div>
									{/if}
								</div>
							</div>
							...
							<div class="grid gap-6 sm:grid-cols-2">
								<div class="space-y-2">
									...
									<div class="flex items-center gap-2">
										<Badge
											variant={getVerificationStatus() === "VERIFIED" ? "default" : "secondary"}
											class="px-3 py-1 text-xs font-medium {getVerificationStatus() === 'VERIFIED'
												? 'bg-green-600 hover:bg-green-700'
												: 'bg-yellow-600 hover:bg-yellow-700'}"
										>
										...
										{#if getVerificationStatus() === "VERIFIED"}
											<span class="text-xs text-green-600">● Licensed professional</span>
										{:else}
											<span class="text-xs text-yellow-600">● Awaiting verification</span>
										{/if}
									</div>
								</div>

								<div class="space-y-2">
									<Label class="text-sm font-medium">Agent Since</Label>
									<div
										class="rounded-lg border bg-muted/30 px-4 py-3 text-sm transition-all duration-200 hover:scale-[1.01] hover:bg-muted/50"
									>
